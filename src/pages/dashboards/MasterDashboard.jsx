...
function PanelTiposClase({ onCambio }) {
  ...
  const toggleDesactivar = async (tipo) => {
    try {
      await api.patch(`/tipos-clase/${tipo._id}/${tipo.activa ? 'desactivar' : 'activar'}`)
      cargar(); onCambio?.()
        ...
        {tipos.map(t => (
          <div key={t._id} className={`tipo-card${!t.activa ? ' tipo-card--inactiva' : ''}`}>
            ...
            <div className="tipo-card__acciones">
              ...
              <button className="turno-card__btn turno-card__btn--cancelar" onClick={() => eliminar(t)}>
                Eliminar
              </button>
            </div>
          </div>
        ...
        <div className="metricas-chart">
          {ingresosChart.map((m, i) => (
            <div key={i} className="metricas-chart__col">
              <span className="metricas-chart__monto">${m.total?.toLocaleString('es-AR') ?? '0'}</span>
              <div className="metricas-chart__barra-wrap">
                <div className="metricas-chart__barra" style={{ height: `${Math.round((m.total / maxIngreso) * 100)}%` }} />
              </div>
      ...
      <div className="metricas-bloque">
        ...
        <div className="metricas-progreso-wrap">
          <div className="metricas-progreso">
            <div className="metricas-progreso__fill" style={{ width: `${ocupacionPromedio}%` }} />
          </div>
...
export default function MasterDashboard() {
  ...
  const cerrarForm = () => { setMostrarFormTurno(false); setTurnoEditar(null) }

  const handleTurnoGuardado = () => { cerrarForm(); cargarTurnos() }
  ...
  const handleAsignarInstructor = async (instructorId) => {
    try {
      const res = await api.put(`/turnos/${turnoSeleccionado._id}`, { instructor: instructorId || null })
  ...
  return (
    <div className="dashboard">
      ...
      <main className="dashboard__main">
        ...
        <h1 className="auth-card__title" style={{ marginBottom: 24 }}>Control total</h1>
